...
import { STANDARD_LIST_PLAYERS } from '@app/constants/development-constants';
...
describe('BoardGameGraph - findReachableNodes', () => {
    ...
    it('should block walls and tiles with players', () => {
        const boardMap: Tile[][] = [
            ...
            [
                { type: TileType.Grass, shortestDistanceFromPosition: [] },
                { type: TileType.Grass, containedPlayer: STANDARD_LIST_PLAYERS[0], shortestDistanceFromPosition: [] },
            ],
        ];
        const speedValue = 3;
        ...
        expect(positions).to.deep.equal([{ x: 1, y: 0 }]);
    });
    ...
    it('should allow movement through a closed door when ignoreDoorStateForVP is true', () => {
        const boardGame: Tile[][] = [
            [
                { type: TileType.Grass, shortestDistanceFromPosition: [] },
                { type: TileType.Door, doorState: false, shortestDistanceFromPosition: [] },
        ...
        const speedValue = 3;
        const graph = new BoardGameGraph(boardGame, true);
        const result = graph.findReachableNodes({ x: 0, y: 0 }, speedValue);
        const positions = extractPositions(result);
