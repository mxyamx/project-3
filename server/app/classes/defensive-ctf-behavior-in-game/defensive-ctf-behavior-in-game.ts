...
export class DefensiveCTFBehaviorInGame extends CTFBehaviorInGame {
    ...
    protected isPositionOccupied(position: Position, activeVirtualPlayer: VirtualPlayer): boolean {
        return this.gameState.listOfPlayers.some(
            (player) =>
                player.position && player.position.x === position.x && player.position.y === position.y && player.name !== activeVirtualPlayer.name,
        );
    }
    ...
    private handleEnemyWithFlag(vpSocket: VpSocketManager, gameId: string, activeVirtualPlayer: VirtualPlayer, playerWithFlag: Player): boolean {
        if (playerWithFlag.startPosition && this.isPositionOccupied(playerWithFlag.startPosition, activeVirtualPlayer)) {
            const playerAtStart = this.findPathToPlayerAtPosition(activeVirtualPlayer, playerWithFlag.startPosition);
            if (playerAtStart) {
                this.vpState.isMovingToItem = false;
                this.moveVirtualPlayer(vpSocket, playerAtStart.path, gameId, this.vpState.isMovingToItem);
                return true;
            ...
            const pathToStart = this.findPathToPlayerWithFlagStartPoint(activeVirtualPlayer);
            if (pathToStart) {
                this.vpState.isMovingToItem = false;
                this.moveVirtualPlayer(vpSocket, pathToStart, gameId, this.vpState.isMovingToItem);
                return true;
